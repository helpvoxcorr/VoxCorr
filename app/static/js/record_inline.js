...
const recorder = new VoxRecorder({
  onTranscript: (f, i) => {
    el('transcription').value = f;
    el('interim').textContent = i;
  },
  onAudioReady: (blob) => {
    audioBlob = blob;
    show('btn-save');
    el('recording-status').textContent = 'Prêt à valider.';
    el('action-bar')?.classList.remove('d-none');
  },
  onStateChange: (s) => {
    if (s === 'recording') {
      hide('btn-start'); show('btn-stop'); hide('btn-save');
      el('recording-status').textContent = '⬤ En cours…';
    }
    if (s === 'stopped') {
...
el('btn-save').onclick = async () => {
  ...
  const scores = [...document.querySelectorAll('.score-input')]
    .map(i => ({ question_id: +i.dataset.qid, score: +i.value || 0 }))
    .filter(s => s.score > 0);

  hide('btn-save');
  el('recording-status').textContent = 'Analyse Mistral…';

  const res = await fetch('/teacher/api/correction/save', {
    method:  'POST',
  ...
  if (audioBlob) {
    ...
    fd.append('audio', audioBlob, 'c.webm');
    fetch('/teacher/api/correction/' + correctionId + '/audio', {
      method:  'POST',
  ...
  pollTimer = setInterval(async () => {
    const d = await fetch('/teacher/api/correction/' + correctionId + '/status')
      .then(r => r.json())
      .catch(() => null);
    ...
    if (d.structured_text) {
      show('ai-result-section');
      el('ai-formatted-text').innerHTML = d.structured_text;
    }
...
el('btn-publish').onclick = async () => {
  const res = await fetch('/teacher/api/correction/' + correctionId + '/publish', {
    method:  'POST',
    headers: { 'X-CSRFToken': _csrf() },
  }).then(r => r.json());
  ...
  hide('btn-publish');
  show('qr-panel');
  el('qr-img').src      = res.qr;
  el('qr-url').href     = res.url;
  el('qr-url').textContent = res.url;
  const dlBtn = document.getElementById('btn-dl-qr');
  if (dlBtn) dlBtn.href = '/teacher/correction/' + correctionId + '/qr.png';
